...
1. **원본 수집** — 폴더에서 스크린샷을 찾고, 없으면 Claude가 **직접 앱을 조작해서** 캡처 (Android는 adb tap/screencap, iOS 시뮬레이터는 딥링크 `simctl openurl` 우선 → idb·Maestro·AppleScript. 상태바는 9:41·배터리 100%로 통일). 사용자는 화면을 넘길 필요 없음 — 선택지 고르기와 문구 컨펌만 하면 됨. 재실행이면 "기존 스크린샷 재활용 / 처음부터 새로"를 먼저 물어봄
